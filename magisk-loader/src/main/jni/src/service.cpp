...
public:
  ...
  static void Init(const SandHook::ElfImg *binder) {
    ...
    getCallingUidFn =
        reinterpret_cast<decltype(getCallingUidFn)>(binder->getSymbAddress(
            "_ZNK7android14IPCThreadState13getCallingUidEv"));
    LOGD("libbinder selfOrNull {} getCallingUid {}", (void *)selfOrNullFn,
         (void *)getCallingUidFn);
  }
};
...
jboolean Service::exec_transact_replace(jboolean *res, JNIEnv *env,
  ...
  if (code == BRIDGE_TRANSACTION_CODE) [[unlikely]] {
    *res =
        JNI_CallStaticBooleanMethod(env, instance()->bridge_service_class_,
...
jboolean Service::call_boolean_method_va_replace(JNIEnv *env, jobject obj,
                                                 jmethodID methodId,
                                                 va_list args) {
  ...
  if (auto self = IPCThreadState::selfOrNull(); self != nullptr) {
    ...
    if (appId >= 10000 && appId <= 19999 && access_matrix != nullptr) {
      need_skip = (access_matrix[(appId - 10000) >> 3] &
                   (1 << ((appId - 10000) & 7))) == 0;
    } else {
      // isolated
  ...
  if (!need_skip && methodId == instance()->exec_transact_backup_methodID_)
      [[unlikely]] {
    jboolean res = false;
    if (exec_transact_replace(&res, env, obj, args)) [[unlikely]]
      return res;
...
LSP_DEF_NATIVE_METHOD(void, BridgeService, initializeAccessMatrix,
                      jobject shared_memory) {
  if (access_matrix != nullptr)
    return;
...
void Service::InitService(JNIEnv *env) {
  ...
  // ServiceManager
  if (auto service_manager_class =
          JNI_FindClass(env, "android/os/ServiceManager")) {
    service_manager_class_ = JNI_NewGlobalRef(env, service_manager_class);
  } else
    return;
  get_service_method_ =
      JNI_GetStaticMethodID(env, service_manager_class_, "getService",
  ...
  if (auto ibinder_class = JNI_FindClass(env, "android/os/IBinder")) {
    transact_method_ =
        JNI_GetMethodID(env, ibinder_class, "transact",
  ...
  write_interface_token_method_ = JNI_GetMethodID(
  ...
  write_string_method_ = JNI_GetMethodID(env, parcel_class_, "writeString",
                                         "(Ljava/lang/String;)V");
  write_strong_binder_method_ = JNI_GetMethodID(
      env, parcel_class_, "writeStrongBinder", "(Landroid/os/IBinder;)V");
  read_exception_method_ =
      JNI_GetMethodID(env, parcel_class_, "readException", "()V");
  ...
  read_strong_binder_method_ = JNI_GetMethodID(
      env, parcel_class_, "readStrongBinder", "()Landroid/os/IBinder;");
  read_string_method_ =
      JNI_GetMethodID(env, parcel_class_, "readString", "()Ljava/lang/String;");
  read_file_descriptor_method_ =
      JNI_GetMethodID(env, parcel_class_, "readFileDescriptor",
                      "()Landroid/os/ParcelFileDescriptor;");
  //        createStringArray_ = env->GetMethodID(parcel_class_,
  //        "createStringArray",
  //                                              "()[Ljava/lang/String;");

  if (auto parcel_file_descriptor_class =
          JNI_FindClass(env, "android/os/ParcelFileDescriptor")) {
    parcel_file_descriptor_class_ =
        JNI_NewGlobalRef(env, parcel_file_descriptor_class);
  } else {
    LOGE("ParcelFileDescriptor not found");
    return;
  }
  detach_fd_method_ =
      JNI_GetMethodID(env, parcel_file_descriptor_class_, "detachFd", "()I");

  if (auto dead_object_exception_class =
          JNI_FindClass(env, "android/os/DeadObjectException")) {
    deadObjectExceptionClass_ =
        JNI_NewGlobalRef(env, dead_object_exception_class);
  }
  initialized_ = true;
}
...
std::string GetBridgeServiceName() {
  ...
  static auto signature =
      obfs_map.at("org.lsposed.lspd.service.") + "BridgeService";
  return signature;
}
...
void Service::HookBridge(const Context &context, JNIEnv *env) {
  ...
  if (!initialized_) [[unlikely]]
    return;
  kHooked = true;
  if (auto bridge_service_class =
          context.FindClassFromCurrentLoader(env, GetBridgeServiceName()))
  ...
  exec_transact_replace_methodID_ = JNI_GetStaticMethodID(
      env, bridge_service_class_, "execTransact", hooker_sig);
  ...
  exec_transact_backup_methodID_ =
      JNI_GetMethodID(env, binder_class, "execTransact", "(IJJI)Z");

  // Android 16+ may have different symbol mangling - try multiple resolution
  // strategies
  auto artImg = SandHook::ElfImg("/libart.so");
  auto *setTableOverride =
      artImg.getSymbAddress<void (*)(JNINativeInterface *)>(
          "_ZN3art9JNIEnvExt16SetTableOverrideEPK18JNINativeInterface");

  // Fallback: try prefix match for potential symbol name changes in Android 16+
  if (!setTableOverride) {
    setTableOverride =
        artImg.getSymbPrefixFirstAddress<void (*)(JNINativeInterface *)>(
            "_ZN3art9JNIEnvExt16SetTableOverride");
    if (setTableOverride) {
      LOGD("SetTableOverride found via prefix match");
    }
  }
  ...
  memcpy(&native_interface_replace_, env->functions,
         sizeof(JNINativeInterface));

  call_boolean_method_va_backup_ = env->functions->CallBooleanMethodV;
  native_interface_replace_.CallBooleanMethodV =
      &call_boolean_method_va_replace;

  if (setTableOverride != nullptr) {
    setTableOverride(&native_interface_replace_);
  }
  if (auto activity_thread_class =
          JNI_FindClass(env, "android/app/IActivityManager$Stub")) {
    if (auto *set_activity_controller_field =
            JNI_GetStaticFieldID(env, activity_thread_class,
                                 "TRANSACTION_setActivityController", "I")) {
      SET_ACTIVITY_CONTROLLER_CODE = JNI_GetStaticIntField(
          env, activity_thread_class, set_activity_controller_field);
    }
  }
  ...
  JNINativeMethod m[] = {LSP_NATIVE_METHOD(
      BridgeService, initializeAccessMatrix, "(Landroid/os/SharedMemory;)V")};
...
ScopedLocalRef<jobject> Service::RequestBinder(JNIEnv *env, jstring nice_name) {
  ...
  auto bridge_service = JNI_CallStaticObjectMethod(
      env, service_manager_class_, get_service_method_, bridge_service_name);
  if (!bridge_service) {
  ...
  auto res = JNI_CallBooleanMethod(env, bridge_service, transact_method_,
                                   BRIDGE_TRANSACTION_CODE, data, reply, 0);
...
ScopedLocalRef<jobject> Service::RequestSystemServerBinder(JNIEnv *env) {
  ...
  auto bridge_service_name =
      JNI_NewStringUTF(env, SYSTEM_SERVER_BRIDGE_SERVICE_NAME);
  ScopedLocalRef<jobject> binder{env, nullptr};
  for (int i = 0; i < 3; ++i) {
    binder = JNI_CallStaticObjectMethod(
        env, service_manager_class_, get_service_method_, bridge_service_name);
    if (binder) {
...
ScopedLocalRef<jobject> Service::RequestApplicationBinderFromSystemServer(
    JNIEnv *env, const ScopedLocalRef<jobject> &system_server_binder) {
  auto heart_beat_binder = JNI_NewObject(env, binder_class_, binder_ctor_);
  ...
  JNI_CallVoidMethod(env, wrapper.data, write_string_method_,
                     JNI_NewStringUTF(env, "system"));
  JNI_CallVoidMethod(env, wrapper.data, write_strong_binder_method_,
                     heart_beat_binder);
  ...
  if (res) {
    ...
    app_binder =
        JNI_CallObjectMethod(env, wrapper.reply, read_strong_binder_method_);
  }
  if (app_binder) {
    JNI_NewGlobalRef(env, heart_beat_binder);
  }
...
Service::RequestLSPDex(JNIEnv *env, const ScopedLocalRef<jobject> &binder) {
  Wrapper wrapper{env, this};
  ...
  auto parcel_fd =
      JNI_CallObjectMethod(env, wrapper.reply, read_file_descriptor_method_);
  int fd = JNI_CallIntMethod(env, parcel_fd, detach_fd_method_);
  auto size = static_cast<size_t>(
      JNI_CallLongMethod(env, wrapper.reply, read_long_method_));
  LOGD("fd={}, size={}", fd, size);
  return {fd, size};
}

std::map<std::string, std::string>
Service::RequestObfuscationMap(JNIEnv *env,
                               const ScopedLocalRef<jobject> &binder) {
  ...
  auto get_string = [this, &wrapper, &env]() -> std::string {
    auto s = JNI_Cast<jstring>(
        JNI_CallObjectMethod(env, wrapper.reply, read_string_method_));
    return JUTFString(s);
  };
